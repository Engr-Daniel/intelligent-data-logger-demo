...
M6 addresses exactly the two capability boundaries exposed by M5:
...
The optional grid-outage/islanding stretch scenario remains outside the frozen M1 scope. The experiment brief itself labels it optional even though an older success-criteria sentence says "all six scenarios".
...
- all five frozen-scope scenarios: PASS on every applicable M5 dimension;
- sensor dropout: PASS on detection, localization, diagnosis/evidence, calibration and abstention;
- all ten canonical query evidence paths: PASS;
- physical validation: PASS;
- automated suite: 70/70 PASS.
